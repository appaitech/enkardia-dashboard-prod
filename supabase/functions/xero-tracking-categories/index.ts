...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Getting tracking categories for tenant: ${tenantId}`);
    ...
    // Fetch the tracking categories from Xero
    const trackingUrl = "https://api.xero.com/api.xro/2.0/TrackingCategories";
    
    console.log(`Calling Xero API: ${trackingUrl}`);
    ...
    if (!trackingResponse.ok) {
      ...
      console.error(`Error fetching tracking categories from Xero:`, errorText);
      console.error(`Status: ${trackingResponse.status} ${trackingResponse.statusText}`);
      
      throw new Error(`Failed to fetch tracking categories from Xero: ${trackingResponse.status} ${trackingResponse.statusText}`);
    }
    
    const trackingData = await trackingResponse.json();
    console.log(`Successfully retrieved tracking categories from Xero`);
    
    return new Response(
      JSON.stringify({ 
        success: true, 
        data: trackingData.TrackingCategories,
        tenantId
      }),
      { headers: { ...corsHeaders, "Content-Type": "application/json" } }
    );
  } catch (error) {
    console.error("Error in Xero tracking categories function:", error.message);
